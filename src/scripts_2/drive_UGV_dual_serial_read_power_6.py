...
def write_data_to_file_and_publish():
    ...
    if power_data:
        ...
        if len(rpm_values) >= 2:  # Ensure there are at least two values
            ...
            try:
                rpm_right_read = int(cleaned_rpm_values[0].strip()) * (2320/1300)  # Convert to int 2320 is 116 rpm and 1300 is motor configuration inside the driver
                rpm_left_read = int(cleaned_rpm_values[1].strip()) * (2320/1300)  # Convert to int 2320 is 116 rpm and 1300 is motor configuration inside the driver
                ...
                Right_force = (48*30*current_right_read)/(3.14*rpm_right_read)
                Left_force = (48*30*current_left_read)/(3.14*rpm_left_read)
